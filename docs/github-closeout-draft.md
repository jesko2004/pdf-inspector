...
更新日期：2026-09-10。文案已按最终本地候选重写；负责人已选择路径 B，本次外部回归单次豁免生效。套件仍未运行；下方描述用于收尾 PR，已创建 [PR #12](https://github.com/jesko2004/pdf-inspector/pull/12)，目标 main，尚未合并。
...
- 已通过现有 Git 凭据推送，再使用同一凭据管理器认证创建 PR #12。修复提交为 `00429a3521c851c4cee7a22a0dcd3dcf56af6bd0`，PR 文件范围与 35 项清单一致；没有执行合并或版本发布。
...
可共享复现说明、提交清单、标题和描述均已准备。外部 pdf-evals 未运行，负责人已批准本次单次豁免。本地收尾提交、推送和 PR #12 创建均已完成，正式审查与合并仍由后续流程处理。真实模型质量、生产 SLA、通用图像文字及完整手写/旋转印章识别不在本轮通过范围。
